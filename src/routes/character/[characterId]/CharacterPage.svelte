<script lang="ts">
	...
	import type { StoredCard } from '$lib/domain/cards/cardStore.svelte.js';
	...
	type EditMode = 'play' | 'content' | 'layout' | 'edit' | (string & {});

	////////////////////////
	// DECK
	let deck: StoredDeck | undefined = $state(character.deck ?? fallbackDeck);
	...
	let lastCharacterState: PrismaCharacter = $state(character.toPrisma());

	function checkSavedState(char: StoredCharacter) {
		console.log('Checking saved state...');
		const { deck: _currentDeck, ...currentWithoutDeck } = char.toPrisma();
	...
	// Track character changes for unsaved state
	$effect(() => {
		character.toPrisma();
		checkSavedState(character);
	});
	...
	$effect(() => {
		...
		if (window && activeCharacter && activeCharacter.id !== characterID) {
			...
			if (confirm) {
				window.location.href = `/character/${activeCharacter.id}`;
			}
		}
	});
	...
	function deleteCharacter(char: PrismaCharacter) {
		if (confirm('Are you sure you want to delete this character?')) {
			spinner.set('delete', 'Deleting...');
			CHARACTER_API.delete(char)
				.then(() => {
	...
	function createCharacter(char: PrismaCharacter) {
		spinner.set('create', 'Creating...');
		CHARACTER_API.create(char)
			.then(() => {
				toast.success('Character created');
				goto(`/character/${char.id}`);
			})
			.catch((error) => {
	...
	function saveCharacter(char: PrismaCharacter) {
		spinner.set('save', 'Saving...');
		CHARACTER_API.update(char)
			.then(() => {
...
<div id="Actions" class="mx-4 my-4 flex flex-row items-center gap-8">
	...
	<div id="contextActions" class="ml-auto flex flex-grow flex-wrap items-center justify-end gap-2">
		...
		{/if}
		<!-- Delete Button -->
		{#if editMode === 'edit' && !isNewCharacter}
			<Button
				size="icon"
...
{#if isNewCharacter || editMode === 'edit'}
	<div id="editCharacterGrid" class="mx-auto grid max-w-[1200px] grid-cols-[auto_1fr] px-2">
		<div
			id="editBreadcrumbs"
			class="sticky top-24 mr-4 flex max-h-[calc(100vh-7rem)] flex-col overflow-y-auto border-r-1 border-threat-500 pr-4 pl-2"
		>
			{#each Object.entries(listedAccordionItems) as [systemKey, propertyComponents]}
				...
				{#each Object.entries(propertyComponents) as [propertyKey, content]}
					<Button
						size="sm"
						variant={openAccordionItems.includes(`${systemKey}:${propertyKey}`) ? 'bold' : 'ghost'}
						class="justify-start"
						onclick={() => {
