...
2. Add the component to the `index.ts` file of the system you want to add it to. Make sure the component itself is both exported as "default", as well as added to the `type ...Options` and the exported `const ...ComponentMap`
3. If you added a new system. Make sure to add an `index.ts` file like the other ones and add both the imported components, as well as the map to the `Deck.svelte` component.
